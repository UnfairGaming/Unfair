...
public class ItemESP extends Module {
    ...
    public final BooleanProperty itemCount = new BooleanProperty("ItemCount", true);
    ...
    public void onRender(Render3DEvent event) {
        if (this.isEnabled()) {
            ...
            for (Entry<ItemData, Integer> itemEntry : itemMap.entrySet().stream().sorted((entry1, entry2) -> {
                ...
                double x = itemEntry.getKey().x - mc.getRenderManager().getRenderPosX();
                double y = itemEntry.getKey().y - mc.getRenderManager().getRenderPosY();
                double z = itemEntry.getKey().z - mc.getRenderManager().getRenderPosZ();
