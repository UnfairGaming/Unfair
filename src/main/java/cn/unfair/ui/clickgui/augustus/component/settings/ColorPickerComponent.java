...
public class ColorPickerComponent extends Component {
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        RenderUtil.drawRect(pickerX, pickerY, pickerW, pickerH, Color.getHSBColor(hue, 1.0F, 1.0F).getRGB());

        for (int ix = 0; ix < pickerW; ix++) {
            float satValue = ix / pickerW;
            int alpha = (int) (255 * (1 - satValue));
            RenderUtil.drawRect(pickerX + ix, pickerY, 1.0F, pickerH, new Color(255, 255, 255, alpha).getRGB());
        }

        for (int iy = 0; iy < pickerH; iy++) {
            float briValue = 1 - (iy / pickerH);
            int alpha = (int) (255 * (1 - briValue));
            RenderUtil.drawRect(pickerX, pickerY + iy, pickerW, 1.0F, new Color(0, 0, 0, alpha).getRGB());
        }
        ...
        for (int ix = 0; ix < pickerW; ix++) {
            float h = ix / pickerW;
            RenderUtil.drawRect(pickerX + ix, hueY, 1.0F, hueH, Color.getHSBColor(h, 1.0F, 1.0F).getRGB());
        }

        float hx = pickerX + hue * pickerW;
        RenderUtil.drawRect(hx - 1.0F, hueY - 1.0F, 2.0F, hueH + 2.0F, Color.WHITE.getRGB());

        float cx = pickerX + sat * pickerW;
        float cy = pickerY + (1 - bri) * pickerH;
        RenderUtil.drawRect(cx - 2.0F, cy - 2.0F, 4.0F, 4.0F, Color.WHITE.getRGB());

        float previewX = pickerX + pickerW + 10.0F;
        RenderUtil.drawRect(previewX, pickerY, 18.0F, 18.0F, (0xFF000000 | property.getValue()));
