...
public class SoundHandler implements IResourceManagerReloadListener, ITickable {
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        ...
        for (String s : resourceManager.getResourceDomains()) {
            try {
                for (IResource iresource : resourceManager.getAllResources(ResourceLocation.of(s, "sounds.json"))) {
                    try {
                        Map<String, SoundList> map = this.getSoundMap(iresource.getInputStream());

                        for (Entry<String, SoundList> entry : map.entrySet()) {
                            this.loadSoundResource(ResourceLocation.of(s, entry.getKey()), entry.getValue());
                        }
            ...
            switch (soundlist$soundentry.getSoundEntryType()) {
                ...
                case SOUND_EVENT:
                    isoundeventaccessor = new ISoundEventAccessor<>() {
                        final ResourceLocation field_148726_a = ResourceLocation.of(s1, soundlist$soundentry.getSoundEntryName());
     ...
     * Play a sound
     */
    public ResourceLocation playSound(ISound sound) {
