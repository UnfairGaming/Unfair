...
import net.minecraft.rendering.optimization.normal.FixedEntityLookHelper;
import net.minecraft.rendering.phosphor.api.ILightingEngineProvider;
import net.minecraft.rendering.phosphor.lighting.LightingEngine;
...
public abstract class World implements IBlockAccess, ILightingEngineProvider {
    ...
    private IBlockState getBlockStateForRayTrace(BlockPos blockPos) {
        if (Unfair.moduleManager != null) {
            AntiObbyTrap antiObbyTrap = (AntiObbyTrap) Unfair.moduleManager.modules.get(AntiObbyTrap.class);

            if (antiObbyTrap.isEnabled() && antiObbyTrap.isInsideBlock(this, blockPos)) {
    ...
    private boolean isPushedByWaterForAcceleration(Entity entity) {
        if (entity == Minecraft.getMinecraft().thePlayer && Unfair.moduleManager != null) {
    ...
    public MovingObjectPosition rayTraceBlocks(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
            if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
                ...
                IBlockState iblockstate = this.getBlockStateForRayTrace(blockpos);
                ...
                while (k1-- >= 0) {
                    ...
                    IBlockState iblockstate1 = this.getBlockStateForRayTrace(blockpos);
    ...
    public boolean spawnEntityInWorld(Entity entityIn) {
        if (entityIn instanceof EntityLiving living) {
            ...
            if (living.getLookHelper() == null || living.getLookHelper().getClass() == EntityLookHelper.class) {
                ...
                living.getLookHelper().posX = oldHelper.posX;
                living.getLookHelper().isLooking = oldHelper.isLooking;
                living.getLookHelper().deltaLookPitch = oldHelper.deltaLookPitch;
                living.getLookHelper().deltaLookYaw = oldHelper.deltaLookYaw;
            }
        }
    ...
    public void updateEntities() {
        ...
        for (final Entity entity2 : this.loadedEntityList) {
            if (entity2 == null || mc == null || mc.thePlayer == null) {
                continue;
            }
    ...
    public <T extends Entity> List<T> getEntities(Class<? extends T> entityType, Predicate<? super T> filter) {
        ...
        for (Entity entity : this.loadedEntityList) {
            if (entityType.isInstance(entity)) {
                T typedEntity = entityType.cast(entity);
                if (filter.apply(typedEntity)) {
    ...
    public <T extends Entity> List<T> getPlayers(Class<? extends T> playerType, Predicate<? super T> filter) {
        ...
        for (Entity entity : this.playerEntities) {
            if (playerType.isInstance(entity)) {
                T typedPlayer = playerType.cast(entity);
                if (filter.apply(typedPlayer)) {
                    list.add(typedPlayer);
                }
            }
        }
    ...
    public CrashReportCategory addWorldInfoToCrashReport(CrashReport report) {
        ...
        try {
            ...
            crashreportcategory.addCrashSectionThrowable("Level data unavailable", throwable);
        }
