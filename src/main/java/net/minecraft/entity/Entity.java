...
import net.minecraft.rendering.EntityLighter;
...
import net.minecraft.rendering.optimization.normal.PooledMutableBlockPos;
...
public abstract class Entity implements ICommandSender, Cullable {
    ...
    public void setAngles(float yaw, float pitch) {
        if (this instanceof EntityPlayerSP && Unfair.rotationManager != null && Unfair.rotationManager.isRotated()) {
            return;
        }

        FreeLook freeLook;

        if (this == Minecraft.getMinecraft().thePlayer && (freeLook = FreeLook.INSTANCE) != null && freeLook.isActive()) {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof EntityPlayerSP) {
                ...
                flag = event.isSafeWalk();
            }

            if (flag) {
                double d6;
                double maxStepDown = ViaProtocol.newerThanOrEqualTo(ProtocolVersion.v1_11)
                        ? -(double) this.stepHeight
                        : -1.0D + 1.0E-7D;

                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, maxStepDown, 0.0D)).isEmpty(); d3 = x) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
                ...
                for (; z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(0.0D, maxStepDown, z)).isEmpty(); d5 = z) {
                    if (z < d6 && z >= -d6) {
                        z = 0.0D;
                    } else if (z > 0.0D) {
                        z -= d6;
                ...
                for (; x != 0.0D && z != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, maxStepDown, z)).isEmpty(); d5 = z) {
                    if (x < d6 && x >= -d6) {
                        x = 0.0D;
                    } else if (x > 0.0D) {
                        x -= d6;
