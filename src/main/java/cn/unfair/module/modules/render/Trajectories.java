...
public class Trajectories extends Module {
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.isEnabled() && mc.thePlayer.getHeldItem() != null && mc.gameSettings.thirdPersonView == 0) {
            ...
            double x = renderManager.getRenderPosX() - (double) MathHelper.cos(yaw / 180.0F * (float) Math.PI) * 0.16;
            double y = renderManager.getRenderPosY() + (double) mc.thePlayer.getEyeHeight() - 0.1F;
            double z = renderManager.getRenderPosZ() - (double) MathHelper.sin(yaw / 180.0F * (float) Math.PI) * 0.16;
