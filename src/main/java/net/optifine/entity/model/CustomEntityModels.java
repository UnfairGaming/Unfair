...
public class CustomEntityModels
{
            ...
            if (Config.isCustomEntityModels())
            {
                ...
                for (int i = 0; i < aresourcelocation.length; ++i)
                {
                    ...
                    if (ientityrenderer != null)
                    {
                        ...
                        if (oclass != null)
                        {
                            if (ientityrenderer instanceof Render && Entity.class.isAssignableFrom(oclass))
                            {
                                map.put(oclass.asSubclass(Entity.class), asEntityRender(ientityrenderer));
                            }
                            else if (ientityrenderer instanceof TileEntitySpecialRenderer && TileEntity.class.isAssignableFrom(oclass))
                            {
                                map1.put(oclass.asSubclass(TileEntity.class), asTileEntityRenderer(ientityrenderer));
                            }
                            else
                            {
    ...
    private static Render<? extends Entity> asEntityRender(IEntityRenderer renderer)
    {
        return (Render<? extends Entity>) renderer;
    }
    ...
    @SuppressWarnings("unchecked")
    private static TileEntitySpecialRenderer<? extends TileEntity> asTileEntityRenderer(IEntityRenderer renderer)
    {
        return (TileEntitySpecialRenderer<? extends TileEntity>) renderer;
    }
    ...
    private static ResourceLocation[] getModelLocations()
    {
        ...
        for (int i = 0; i < astring.length; ++i)
        {
            String s2 = astring[i];
            String s3 = s + s2 + s1;
            ResourceLocation resourcelocation = ResourceLocation.of(s3);
