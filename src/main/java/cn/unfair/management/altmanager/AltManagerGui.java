...
public class AltManagerGui extends GuiScreen {
    ...
    public void handleMouseInput() {
        ...
        if (wheel != 0 && dialog == Dialog.NONE && inside((double) (Mouse.getEventX() * this.width) / mc.displayWidth, this.height - (double) (Mouse.getEventY() * this.height) / mc.displayHeight - 1, listX, listY, listW, listH)) {
    ...
    private void loginSelected() {
        ...
        if (selected.isCracked()) {
            ...
            loginSelectedWithRefreshToken(selected);
        } else {
    ...
    private void loginSelectedWithRefreshToken(Alt alt) {
        status = "§6Logging in with OAuth...";
        new Thread(() -> {
            MicrosoftOAuthTranslation.LoginData loginData = MicrosoftOAuthTranslation.login(alt.getRefreshToken());
            mc.addScheduledTask(() -> {
                if (loginData.isGood()) {
                    SessionChanger.instance().setSessionWithData(loginData);
                    updateAltFromOAuth(alt, loginData);
                    selected = alt;
                    saveAlts();
                    status = "§aLogged in as " + loginData.username;
                } else {
                    SessionChanger.instance().timeSinceFail = System.currentTimeMillis();
                    status = "§c" + (loginData.errorMessage != null ? loginData.errorMessage : "OAuth login failed");
                }
            });
        }, "Unfair Alt Login").start();
    }
    ...
    private void accountFromOAuth(MicrosoftOAuthTranslation.LoginData loginData) {
        Alt existing = null;
        for (Alt alt : alts) {
            if (alt.getName() != null && alt.getName().equalsIgnoreCase(loginData.username)) {
                existing = alt;
                break;
            }
        }
        ...
        if (existing != null) {
            updateAltFromOAuth(existing, loginData);
            selected = existing;
        } else {
    ...
    private void updateAltFromOAuth(Alt alt, MicrosoftOAuthTranslation.LoginData loginData) {
        alt.setName(loginData.username);
        alt.setUuid(loginData.uuid);
        alt.setCracked(false);

        if (loginData.newRefreshToken != null && !loginData.newRefreshToken.isEmpty()) {
            alt.setRefreshToken(loginData.newRefreshToken);
        }
    }
    ...
    private boolean isValidOfflineName(String text) {
        if (text == null || text.isEmpty() || text.length() > NAME_LIMIT) {
            return false;
        }
