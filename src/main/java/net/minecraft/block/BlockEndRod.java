...
public class BlockEndRod extends ModernBlock {
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, BlockPos pos) {
        setBounds(world.getBlockState(pos).getValue(FACING));
    }
    ...
    public AxisAlignedBB getCollisionBoundingBox(World world, BlockPos pos, IBlockState state) {
        setBounds(state.getValue(FACING));
    ...
    public boolean handlesViaState(ProtocolVersion protocol, int stateId) {
        return protocol.equals(ProtocolVersion.v1_9) ? stateId >= 3168 && stateId <= 3173 : super.handlesViaState(protocol, stateId);
    }

    public IBlockState getStateFromViaState(ProtocolVersion protocol, int stateId) {
        return protocol.equals(ProtocolVersion.v1_9) ? getDefaultState().withProperty(FACING, LEGACY_FACING[stateId - 3168]) : getStateFromViaStateId(stateId);
    }

    public IBlockState getStateFromViaStateId(int stateId) {
        return getDefaultState().withProperty(FACING, VIA_FACING[stateId - 7997]);
    }
