...
public class ModelBakery {
    ...
    private static final Set<ResourceLocation> LOCATIONS_BUILTIN_TEXTURES = Sets.newHashSet(
            ...
            ResourceLocation.of("blocks/destroy_stage_2"), ResourceLocation.of("blocks/destroy_stage_3"), ResourceLocation.of("blocks/destroy_stage_4"),
    ...
    private void loadVariantItemModels() {
        ...
        this.variants.put(MODEL_MISSING, new ModelBlockDefinition.Variants(MODEL_MISSING.getVariant(), Lists.newArrayList(new ModelBlockDefinition.Variant(ResourceLocation.of(MODEL_MISSING.getResourcePath()), ModelRotation.X0_Y0, false, 1))));
        ResourceLocation resourcelocation = ResourceLocation.of("item_frame");
    ...
    private ResourceLocation getBlockStateLocation(ResourceLocation p_177584_1_) {
        return ResourceLocation.of(p_177584_1_.getResourceDomain(), "blockstates/" + p_177584_1_.getResourcePath() + ".json");
    }
    ...
    private ResourceLocation getModelLocation(ResourceLocation p_177580_1_) {
        ...
        if (!s.startsWith("mcpatcher") && !s.startsWith("optifine")) {
            return ResourceLocation.of(p_177580_1_.getResourceDomain(), "models/" + p_177580_1_.getResourcePath() + ".json");
        } else {
            if (!s.endsWith(".json")) {
                resourcelocation = ResourceLocation.of(p_177580_1_.getResourceDomain(), s + ".json");
            }
    ...
    private void registerVariantNames() {
        ...
        this.variantNames.put(Item.getItemFromBlock(Blocks.dirt_path), Lists.newArrayList("dirt_path", "grass_path"));
    ...
    public IBakedModel bakeModel(ModelBlock modelBlockIn, ModelRotation modelRotationIn, boolean uvLocked) {
        TextureAtlasSprite textureatlassprite = this.sprites.get(ResourceLocation.of(modelBlockIn.resolveTextureName("particle")));
        ...
        for (BlockPart blockpart : modelBlockIn.getElements()) {
            for (EnumFacing enumfacing : blockpart.mapFaces.keySet()) {
                ...
                TextureAtlasSprite textureatlassprite1 = this.sprites.get(ResourceLocation.of(modelBlockIn.resolveTextureName(blockpartface.texture)));
                boolean flag = true;
    ...
    private Set<ResourceLocation> getTextureLocations(ModelBlock p_177585_1_) {
        ...
        for (BlockPart blockpart : p_177585_1_.getElements()) {
            for (BlockPartFace blockpartface : blockpart.mapFaces.values()) {
                ResourceLocation resourcelocation = ResourceLocation.of(p_177585_1_.resolveTextureName(blockpartface.texture));
                set.add(resourcelocation);
            }
        }

        set.add(ResourceLocation.of(p_177585_1_.resolveTextureName("particle")));
        return set;
    }
    ...
    private void loadSprites() {
        ...
        this.sprites.put(ResourceLocation.of("missingno"), this.textureMap.getMissingSprite());
    }
    ...
    private Set<ResourceLocation> getItemsTextureLocations() {
        ...
        for (ResourceLocation resourcelocation : this.itemLocations.values()) {
            ...
            if (modelblock != null) {
                set.add(ResourceLocation.of(modelblock.resolveTextureName("particle")));

                if (this.hasItemModel(modelblock)) {
                    for (String s : ItemModelGenerator.LAYERS) {
                        ResourceLocation resourcelocation2 = ResourceLocation.of(modelblock.resolveTextureName(s));
                    ...
                    for (BlockPart blockpart : modelblock.getElements()) {
                        for (BlockPartFace blockpartface : blockpart.mapFaces.values()) {
                            ResourceLocation resourcelocation1 = ResourceLocation.of(modelblock.resolveTextureName(blockpartface.texture));
