...
public class TokenLoginGui extends GuiScreen {
    ...
    private void loginWithToken(String token) {
        ...
        new Thread(() -> {
            try {
                // If it looks like a JWT, try using it directly as the Minecraft access token.
                if (cleanToken.startsWith("eyJ")) {
                    AltManagerGui.status = "§eAttempting Direct Login...";
                    try {
                        String[] profile = getProfileInfo(cleanToken);
                        handleAccessTokenLoginSuccess(cleanToken, profile[0], profile[1]);
                        return;
                    } catch (IOException e) {
                ...
                if (loginData.isGood()) {
                    handleRefreshTokenLoginSuccess(loginData);
                } else {
    ...
    private void handleAccessTokenLoginSuccess(String token, String username, String uuid) {
    ...
    private void handleRefreshTokenLoginSuccess(MicrosoftOAuthTranslation.LoginData loginData) {
        ...
        mc.addScheduledTask(() -> {
            ...
            Alt existingAlt = null;
            for (Alt alt : AltManagerGui.getAlts()) {
                if (alt.getName() != null && alt.getName().equals(loginData.username)) {
                    existingAlt = alt;
                    break;
                }
            }
            ...
            if (existingAlt != null) {
                ...
                if (loginData.newRefreshToken != null && !loginData.newRefreshToken.isEmpty()) {
                    existingAlt.setRefreshToken(loginData.newRefreshToken);
                }
            } else {
