...
public class GlStateManager {
    ...
    public static void generateTextures(IntBuffer buffer) {
        synchronized (RENDER_THREAD_LOCK) {
    ...
    public static int generateTexture() {
        ...
        if (!Minecraft.getMinecraft().isCallingFromMinecraftThread() && GLFW.glfwGetCurrentContext() <= 0) {
            try {
                return Minecraft.getMinecraft().addScheduledTask((Callable<Integer>) GL11::glGenTextures).get();
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new RuntimeException("Interrupted while generating texture", e);
